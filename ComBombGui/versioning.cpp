...
int32_t parseVersionStr(const std::string& verStr)
{
    ...
    if (vs[0] == 'v')
    {
        ...
        else if (numDots == 2)
        {
            ...
            {
                int32_t year = boost::lexical_cast<int32_t>(text.substr(0, dotIndex0));
                int32_t month = boost::lexical_cast<int32_t>(text.substr(dotIndex0 + 1, dotIndex1));
                int32_t day = boost::lexical_cast<int32_t>(text.substr(dotIndex1 + 1));
